...
- **Nav segmented pill (desktop + mobile + tablet)** — final design: `[ APPLY NOW | MEMBER SITE ]`. `APPLY NOW` = solid cyan `#00A8E1` (primary CTA on left). `MEMBER SITE` = white bg + `#0c2340` text (high contrast against navy header, no outline needed). `whitespace-nowrap` + `leading-none` + `text-sm` prevent wrapping. Container widened to `max-w-[1440px]` with `gap-6`, logo capped at `max-w-[220px]` to fix header overlap.
