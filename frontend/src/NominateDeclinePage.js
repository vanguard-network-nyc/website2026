...
/**
 * Nominate Decline form — reached via Mailchimp email links:
 *   /nominate/decline?email={{EMAIL}}&code=NGGC2027_0201
 * Writes to sheet 1AuA3nIfWuHdm8TpwiS-J4py8zQ3DsBBXI332VRPm2-4 tab "DECLINES".
 */
...
const DeclineForm = ({ initialEmail, eventCode, onSuccess }) => {
    ...
    try {
      const res = await fetch(SUBMIT_URL, {
        ...
        body: JSON.stringify({
          ...
          eventCode: eventCode || DEFAULT_EVENT_CODE,
        }),
      });
      let body = null;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" data-testid="decline-form">
      ...
      {/* Event code — hidden, prefilled from URL or default. */}
      <input
        type="hidden"
        name="eventCode"
        value={eventCode || DEFAULT_EVENT_CODE}
        data-testid="decline-eventCode"
        readOnly
      />
...
const DeclineSuccess = ({ onClose }) => (
  <div className="py-6 text-center" data-testid="decline-success">
    ...
    <button
      type="button"
      onClick={onClose}
      className="group inline-flex items-center gap-3 bg-gradient-to-r from-[#045184] to-[#00A8E1] text-white px-8 py-4 rounded-xl font-bold text-lg transition-all duration-300 hover:shadow-xl hover:scale-105 mt-4"
      data-testid="decline-close-btn"
    >
      Return to Next Gen GC program
...
const NominateDeclinePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100" data-testid="nominate-decline-page">
      <SEO
        title="No Nominee — Vanguard Next Gen GC 2027 Program"
      ...
      <SignupModal isOpen onClose={handleClose} title="No nominee. I don't have a nomination for the Vanguard Next Gen GC 2027 program.">
        {step === 'form' ? (
          ...
          <DeclineSuccess onClose={handleClose} />
        )}
      </SignupModal>
    </div>
  );
