      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.nav
            ...
            style={{ top: '140px' }}
          >
            <div className="px-8 py-5 grid grid-cols-2 gap-x-8">
              {/* Left column: items WITH dropdowns */}
              <div>
                {navItems.filter(i => i.dropdown).map((item, index) => (
                  <motion.div
                    key={item.name}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.05 }}
                    className="border-b border-slate-600/30 py-1 mb-1"
                  >
                    <div className="text-white font-medium text-base lg:text-lg">{item.name}</div>
                    {item.dropdown.map((subItem) => (
                      subItem.external ? (
                        <a
                          key={subItem.name}
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="block text-blue-300 hover:text-blue-100 pl-3 text-sm lg:text-base transition-colors duration-200 py-0.5"
                          onClick={() => setIsMenuOpen(false)}
                        >
                          {subItem.name}
                        </a>
              ...
              {/* Right column: items WITHOUT dropdowns + MEMBER SITE button */}
                  ...
                  >
                    {item.external ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block text-white hover:text-blue-400 font-medium text-base lg:text-lg transition-colors duration-200 py-1"
                        onClick={() => setIsMenuOpen(false)}
                      >
                        {item.name}
                      </a>
...
const AdvisoryCard = ({ advisor, index, onOpen }) => (
  ...
  >
    ...
    <div className="p-4 flex items-center justify-between gap-3">
      <h3 className="text-base font-bold text-slate-900 leading-tight min-w-0 flex-1">{advisor.name}</h3>
      <span className="shrink-0 inline-flex items-center gap-1.5 text-[15px] font-bold text-[#F97316] group-hover:text-[#C2410C] underline underline-offset-4 decoration-2 transition-colors whitespace-nowrap">
...
const ImageSliderSection = () => {
  const sliderImages = [
    ...
    { src: "/gc-forum-2026-4.jpg", alt: "The Vanguard Network General Counsel Forum, May 2026 — keynote discussion" },
    ...
    { src: "/gc-forum-2026-6.jpg", alt: "The Vanguard Network General Counsel Forum, May 2026 — roundtable exchange" },
    ...
    { src: "/gc-forum-2026-7.jpg", alt: "The Vanguard Network General Counsel Forum, May 2026 — leadership dialogue" },
